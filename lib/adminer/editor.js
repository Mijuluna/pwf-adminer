		...
		mod = {
			...
			'proto':{
				...
				'construct_form':function(proto) {
					...
					this.get_el().form = pwf.form.create({
						'ref':this,
						'action':data.url,
						'parent':data.parent,
				...
				'error':function(proto, err, response) {
					this.respond('on_error', [err, ctrl]);
				},

				'saved':function(proto, err, data) {
					var is_new = this.is_new();
					this.get('item').update(data);
					this.respond('after_save', [err, ctrl]);
				}
			},
			...
			'public':{
				...
				{
					this.preload_rels(this.get_attrs(), function(ctrl, proto, next) {
