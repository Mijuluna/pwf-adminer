			...
			'proto':{
				...
				'construct_form':function(p)
				{
					...
					this.get_el().form = pwf.create('form', {
						...
						'on_invalid':data.calls.on_invalid,
						'elements':els
					});
				...
				'get_calls':function(p)
				{
					return {
						'before_send':function(err, form) {
							...
							p.storage.is_new = p.object.is_new();

							if (this.get('before_save') instanceof Function) {
								save = p.object.respond('before_save', [err]);
							}

							return typeof save == 'undefined' || save;
						},
						...
						'on_invalid':function(list) {
							p.object.respond('on_invalid', [list]);
						},
						...
						'on_ready':function(err, data, response) {
							...
							if (!err) {
								p.object.get('item').update(data.data);

								if (p.object.get('reload')) {
									fire = false;

									return p.object.reload(function(data, response) {
										return function(err) {
											p('saved', err, data);
										};
									}(data, response));
