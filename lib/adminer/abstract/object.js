	...
	var
		...
		mod = {
			...
			'proto':{
				...
				'construct_heading':function() {
					this.get_el('heading').html(pwf.jquery.div('heading').html(
						pwf.locales.trans(this.get_heading(), {
							'model_name':pwf.locales.trans_model(this.get('model'), 'dative').text(),
				...
				'preload_rels':function(proto, attrs, next) {
					...
					for (var i = 0; i < attrs.length; i++) {
						...
						if (proto('rel_types').indexOf(rel.type) >= 0) {
							var
								is_natural = rel.cname == obj.cname(),
							...
							for (var i = 0; i < val.length; i++) {
								jobs.push(function(adminer, obj, preload) {
									return function(next) {
										...
										pwf.create('adminer.editor', opts).load(next);
									};
								}(this, val[i], preload));
							}
						}
