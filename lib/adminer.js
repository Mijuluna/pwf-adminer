						...
						if (opt_buttons !== null) {
							for (var i = 0; i < opt_buttons.length; i++) {
								var
									...
									def = pwf.jquery.extend(base, opt_buttons[i]);
						...
						el.form = pwf.form.create({
							...
							'elements':[
								...
								},
								{
									"element":'container',
									'type':'buttons',
									'elements':this.get_buttons()
								}
							],

							'on_before_send':function(ctrl) {
							...
							'on_ready':function(ctrl, el, obj) {
								return function(data, res) {
									obj.update(res.data);

									if (typeof ctrl.ref().get('on_save') == 'function') {
										ctrl.ref().get('on_save')(ctrl, obj);
									}
					...
					this.load = function(next)
					{
						this.ref().preload_rels(obj, obj.model().get_attrs(), function(ctrl, next) {
							return function(err, obj) {
								var warning = ctrl.get_rel_conflicts();
